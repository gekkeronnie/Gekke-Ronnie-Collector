...
function GRC.TrackingBar.CreateBar(item, itemType, columnIndex, rowIndex)
    ...
    if GRCollectorTrackingSettings.showText then
        ...
        local textContent = item.name or "Unknown"
        
        -- Truncate name if too long - adjust based on bar width
        local maxLength = math.floor(barWidth / 8) -- Approximate character width
        if #textContent > maxLength then
            textContent = textContent:sub(1, maxLength - 3) .. "..."
        end
        ...
        if item.isCollected then
            ...
            if attempts > 0 then
                if GRCollectorTrackingSettings.showPercentage then
                    textContent = textContent .. string.format(" (%d attempts)", attempts)
                else
                    textContent = textContent .. string.format(" (%d)", attempts)
                end
            else
                textContent = textContent .. " (Collected)"
            end
        else
            local estimatedNeeded = GRC.TrackingBar.GetEstimatedAttempts(item)
            ...
            if attempts > 0 then
                if GRCollectorTrackingSettings.showPercentage then
                    textContent = textContent .. string.format(" (%d/%d - %d%%)", attempts, estimatedNeeded, percentage)
                else
                    textContent = textContent .. string.format(" (%d)", attempts)
                end
            else
